...
export default function Lenses() {
    ...
    // Вытягиваем уникальные бренды из data.json
    const uniqueBrands = Array.from(new Set(data.lenses.map(item => (item as any).brand))).filter(Boolean);

    // Фильтруем и сортируем
    const filteredAndSortedData = data.lenses
    ...
    return (
        <div className={styles.lenses}>
            ...
            <div className={styles.lenses__container}>
                {filteredAndSortedData.map((lenses) => (
                    <div key={lenses.id} className={styles.lenses__card}>
                        <Image 
                            src={lenses.photo} 
                            alt={`${lenses.name} photo`} 
                            className={styles.lenses__photo} 
                            width={210} 
                            height={170} 
                            quality={100} 
                            onClick={() => goToProduct(lenses.id)}
                            style={{ cursor: 'pointer', objectFit: 'contain' }}
                        />
                        <p className={styles.lenses__name}>{lenses.name}</p>
                        <div className={styles.lenses__price__cart__block}>
                            <p>€{lenses.price}</p>
                            <Button className={styles.lenses__buy__btn} onClick={() => goToProduct(lenses.id)} variant="light">
                                View & Buy
